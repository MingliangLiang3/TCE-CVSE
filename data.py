...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, cap_json_path, data_path, attribute_path, data_split, vocab, opt):
        ...
        if isinstance(cap_json_path, tuple):
            self.coco = (COCO(cap_json_path[0]), COCO(cap_json_path[
                                                          1]))  # load coco data from json file with COCO protocol provided by importing pycocotools.coco
        ...
        self.test_on = opt.test_on

        loc = data_path + '/'

        # Captions
        self.captions = []
        with open(loc + '%s_caps.txt' % self.data_split, 'rb') as f:
            for line in f:
                self.captions.append(line.strip())

        self.concepts_pred = []
        with open('./data/concept_prediction/top_300_concepts_pred.txt', 'rb') as f:
            for line in f:
                self.concepts_pred.append(eval(line.strip()))

        # Image features
        print("Image path", loc + '%s_ims.npy' % self.data_split)
        self.images = np.load(loc + '%s_ims.npy' % self.data_split)
        ...
        self.image_ids = []
        img_file_name = loc + '%s_ids.txt' % self.data_split
        ...
        self.get_anno()
        self.attribute_label = self.get_attribute_label()
    ...
    def get_attribute_label(self):
        ...
        for label in self.img_list:
            attribute_label[label["file_name"]] = label["concept_labels"]
    ...
    def get_anno(self):
        ...
        if isinstance(self.attribute_json_dir, tuple):
            ...
            self.img_list = jsonmod.load(open(list_path, 'r'))  # concept label list
            self.cat2idx = jsonmod.load(open(self.attribute_name_json_dir, 'r'))
    ...
    def __getitem__(self, index):
        ...
        if self.data_split == 'train':
            ...
            attribute_label = self.get(self.attribute_label[img_file_name])
            attribute_label = torch.Tensor(attribute_label)

        elif self.data_split == 'testall' and self.test_on == "prediction":

            # Using predict concept label
            attribute_label = np.zeros(self.num_classes, np.float32)
            if len([self.concepts_pred[index]]) > 0:
                attribute_label[self.concepts_pred[index]] = 1

            attribute_label = torch.Tensor(attribute_label)

        elif self.data_split == 'testall' and self.test_on == "one":
            '''b) generate the real label for each sentence'''
            attribute_label = np.zeros(self.num_classes, np.float32)
            for (i, word) in enumerate(tokens_sent):
                if word in self.name_concepts:
                    inx_concept = self.name_concepts.index(word)
                    attribute_label[inx_concept] = 1
            attribute_label = torch.Tensor(attribute_label)

        elif self.data_split == 'testall' and self.test_on == "five":
            '''b) generate the real label for each sentence'''
            image_id = self.image_ids[img_id]
            coco = self.coco_restval
            img_file_name = coco.loadImgs(image_id)[0]['file_name']
        ...
        elif self.data_split == 'dev' and self.test_on == "one":
            '''b) generate the real label for each sentence'''
            attribute_label = np.zeros(self.num_classes, np.float32)
            for (i, word) in enumerate(tokens_sent):
                if word in self.name_concepts:
        ...
        elif self.data_split == 'dev' and self.test_on == "five":
            '''b) generate the real label for each sentence'''
            image_id = self.image_ids[img_id]
            coco = self.coco_restval
            img_file_name = coco.loadImgs(image_id)[0]['file_name']
            ...
            attribute_label = self.get(self.attribute_label[img_file_name])
            attribute_label = torch.Tensor(attribute_label)
        elif self.data_split == 'dev' and self.test_on == "prediction":
        ...
        attri_input_emb = self.attribue_input_emb;
        attri_input_emb = torch.Tensor(attri_input_emb)
...
class PrecompDataset_Flickr30k(data.Dataset):
    ...
    def __init__(self, data_path, attribute_path, data_split, vocab, opt):
        ...
        self.concepts_pred = []
        with open('f30k_concept_top_300_new_labels.txt', 'rb') as f:
            for line in f:
                self.concepts_pred.append(eval(line.strip()))
    ...
    def get_attribute_label(self):
        attribute_label = {}
        for label in self.img_list:
            attribute_label[int(label["img_id"])] = label["concept_labels"]
        return attribute_label
    ...
    def __getitem__(self, index):
        ...
        # Convert caption (string) to word ids.
        tokens = nltk.tokenize.word_tokenize(str(caption).lower())
        tokens_sent = nltk.tokenize.word_tokenize(sent.lower())
        ...
        if self.data_split == 'train':
            attribute_label = np.zeros(self.num_classes,
                                      np.float32) / self.num_classes  # change for avoiding the empty labels
            # a) load the annoted labels
            attribute_label = self.get(self.attribute_label[img_id])
            attribute_label = torch.Tensor(attribute_label)
        elif self.data_split == 'test' and self.test_on == "prediction":

            # Using predict concept labels
            attribute_label = np.zeros(self.num_classes, np.float32)
            if len([self.concepts_pred[index]]) > 0:
                attribute_label[self.concepts_pred[index]] = 1
            attribute_label = torch.Tensor(attribute_label)

        elif self.data_split == 'test' and self.test_on == "five":
            attribute_label = np.zeros(self.num_classes, np.float32) / self.num_classes
            # a) load the annoted labels
            attribute_label = self.get(self.attribute_label[img_id])
            attribute_label = torch.Tensor(attribute_label)

        elif self.data_split == 'test' and self.test_on == "one":
        #'''b) generate the real label for each sentence'''
            attribute_label = np.zeros(self.num_classes, np.float32)
            for (i, word) in enumerate(tokens_sent):
                try:
            ...
            attribute_label = np.ones(self.num_classes, np.float32) / self.num_classes
            # a) load the annoted labels
            attribute_label = self.get(self.attribute_label[img_id])
            attribute_label = torch.Tensor(attribute_label)
