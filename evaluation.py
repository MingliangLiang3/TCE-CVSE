...
def encode_data(model, data_loader, log_step=200, logging=print, alpha=None):
    ...
    for i, (images, captions, attribute_labels, attribute_input_embs, lengths, ids) in enumerate(data_loader):
        ...
        img_emb, cap_emb, predict_score_v, predict_score_t = model.forward_emb(images, captions,
                                                                               ...
                                                                               lengths, alpha,
                                                                               volatile=True)  # img_emb : instance_emb_v, consensus_emb_v, fused_emb_v
        # with open("data/predict_score_v/np_predict_score_v_b%d.npy" % i, 'wb') as f:
        #     np.save(f, predict_score_v.cpu().detach().numpy())
        #
        # with open("data/predict_score_t/np_predict_score_t_b%d.npy" % i, 'wb') as f:
        #     np.save(f, predict_score_t.cpu().detach().numpy())
        ...
        img_embs_inst[list(ids)] = img_emb_inst.data.cpu().numpy().copy()
        cap_embs_inst[list(ids)] = cap_emb_inst.data.cpu().numpy().copy()
        img_embs_cons[list(ids)] = img_emb_cons.data.cpu().numpy().copy()
        cap_embs_cons[list(ids)] = cap_emb_cons.data.cpu().numpy().copy()
        concept_labels[list(ids)] = attribute_labels.data.cpu().numpy().copy()
        # concept_labels[list(ids)] = attribute_labels.data.cpu().numpy().copy()
        ids_list.append(list(ids))
        # measure accuracy and record loss
        # model.forward_loss(img_emb, cap_emb, predict_score_v, predict_score_t, model.dataset_name, concept_labels)
...
def encode_data_KNN_rerank(model, data_loader, log_step=200, logging=print, index_KNN_neighbour=None,
                           concept_labels=None, alpha=None):
    """
    Encode all images and captions loadable by `data_loader`
    """
    batch_time = AverageMeter()
    ...
    for i, (images, captions, attribute_labels, attribute_input_embs, lengths, ids) in enumerate(data_loader):
        model.logger = val_logger
        Complete_labels = copy.deepcopy(attribute_labels);
        Complete_labels = Complete_labels.numpy()  # convert to array
        ...
        # re-weights by image score
        # cap_emb_inst = cap_emb[0] * predict_score_v
        ...
        img_embs_cons[list(ids)] = img_emb_cons.data.cpu().numpy().copy()
        cap_embs_cons[list(ids)] = cap_emb_cons.data.cpu().numpy().copy()
        Complete_labels_all[list(ids)] = Complete_labels.data.cpu().numpy().copy()
...
def evalrank(model_path, data_path=None, data_name=None, data_name_vocab=None, split='dev', fold5=False,
             VSE_model=None, data_loader=None, concept_path=None, transfer_test=False, concept_name=None, test_on=None,
             use_knn=False):
    """
    ...
    if 'adj_gen_mode' not in vars(opt).keys():
        opt.adj_gen_mode = "ReComplex"
    if 'alpha' not in vars(opt).keys():
        opt.alpha = 0.9
    if 't' not in vars(opt).keys():
        opt.t = 0.3
    if 're_weight' not in vars(opt).keys():
        opt.re_weight = 0.25
    opt.test_on = test_on
    # opt.alpha = 0.85 # fuse weight
    ...
    img_embs, cap_embs, img_emb_cons, cap_emb_cons, concept_labels = encode_data(model=model, data_loader=data_loader,
                                                                                 alpha=opt.alpha)
    ...
    if not fold5:
        ...
        r, rt = i2t_sep_sim(img_embs, cap_embs, img_emb_cons, cap_emb_cons, opt.data_name, weight_fused=0.95,
                            measure=opt.measure, return_ranks=True)
        ri, rti = t2i_sep_sim(img_embs, cap_embs, img_emb_cons, cap_emb_cons, opt.data_name, weight_fused=0.95,
                              measure=opt.measure, return_ranks=True)
        ar = (r[0] + r[1] + r[2]) / 3
        ...
        for i in range(5):
            r, rt0 = i2t_sep_sim(img_embs[i * 5000:(i + 1) * 5000], cap_embs[i * 5000:(i + 1) * 5000],
                                 ...
                                 opt.data_name, weight_fused=opt.alpha, measure=opt.measure, return_ranks=True)
            print("Image to text: %.1f, %.1f, %.1f, %.1f, %.1f" % r)
...
def i2t_sep_sim(images, captions, img_emb_cons, cap_emb_cons, data_name=None, weight_fused=0.8, npts=None,
...
def t2i_sep_sim(images, captions, img_emb_cons, cap_emb_cons, data_name=None, weight_fused=0.8, npts=None,
                measure='cosine', return_ranks=False):
    """
    ...
    for index in range(npts):
        ...
        # weighted sum of two level similarities
        d = weight_fused * d_inst + (1 - weight_fused) * d_cons
        with open("t2i_score.csv", "a") as f:
            for l in range(len(d)):
                f.write(str(list(d[l])).strip("[]") + "\n")
...
def construct_N_with_KNN(mat_sim_inst, K_neigs=3, return_index=True):
    ...
    for i in range(mat_sim_inst.shape[1]):
        ...
        index_i_txt = np.where(cand_nearest_i2t == i);
        index_i_txt = index_i_txt[0][0]
...
def label_complete(concept_label, img_embs, cap_embs, data_name):
    """
    Text-to-text  similarity to predicted concept labels
    :param concept_label:
    :param img_embs:
    :param cap_embs:
    :param data_name:
    ...
    # txt emb KNN search
    # Computing K-nearest Neighbours for all sentences(25000) didn't make sense
    ...
    for i in range(len(index_K_neighbour_t2t)):
        a = index_K_neighbour_t2t[i];
        a = [int(elem) for elem in a]
        b = index_K_neighbour_i2t[i];
