...
class Consensus_level_feature_learning(nn.Module):
    ...
    def forward(self, emb_instance, concept_feature, input_modal, GT_label, GT_label_ratio):
        ...
        if input_modal == 'textual':
            ...
            # weights_u = weights  # remove concept labels
...
class CVSE(object):
    ...
    def __init__(self, word2idx, opt):
        ...
        self.fuse_weight = opt.alpha
        # self.fuse_weight = 1.0
        ...
        # Consensus_GCN
        self.C_GCN = C_GCN(opt.num_attribute, in_channel=opt.input_channel, t=opt.t, adj_file=opt.adj_file,
                           norm_func=opt.norm_func_type, adj_gen_mode=opt.adj_gen_mode, opt=opt)
    ...
    def forward_emb(self, images, captions, concept_labels, concept_input_embs, lengths, alpha, volatile=False):
        ...
        consensus_emb_v, predict_score_v = self.V_consensus_level_embedding(instance_emb_v, concept_basis_v,
                                                                            ...
                                                                            GT_label_ratio=self.GT_label_ratio)  # predict_score_v : weights
        # concept_labels
        consensus_emb_t, predict_score_t = self.T_consensus_level_embedding(instance_emb_t, concept_basis_t,
                                                                            input_modal='textual',
                                                                            GT_label=concept_labels,
                                                                            GT_label_ratio=self.GT_label_ratio)  # predict_score_t : weights
    ...
    def forward_loss(self, v_emb, t_emb, predict_score_v, predict_score_t, dataset_name, concept_labels, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        if 'coco' in dataset_name or 'f30k' in dataset_name:
            weight = [3, 5, 1, 2]  # loss weights for coco
            # weight = [0, 1, 0, 0]  # loss weights for only V^I
        else:
        ...
        # concept_labels = concept_labels.to(predict_score_v.device)
        # v_multi_label_loss = self.criterion(predict_score_v, concept_labels)
        # t_multi_label_loss = self.criterion(predict_score_t, concept_labels)

        loss = weight[0] * loss_rank + weight[1] * loss_rank_instance + weight[2] * loss_rank_consensus + \
               weight[3] * loss_cls_KL + weight[3]  # * v_multi_label_loss + weight[3] * t_multi_label_loss

        # self.logger.update('v_multi_label_loss', v_multi_label_loss.item(), v_emb.size(0))
        # self.logger.update('t_multi_label_loss', t_multi_label_loss.item(), v_emb.size(0))
    ...
    def train_emb(self, images, captions, concept_labels, concept_input_embs, lengths, ids=None, *args):
        ...
        loss = self.forward_loss(v_emb, t_emb, predict_score_v, predict_score_t, self.dataset_name, concept_labels)
