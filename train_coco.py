...
from evaluation import i2t_sep_sim, t2i_sep_sim, AverageMeter, LogCollector, encode_data, encode_data_KNN_rerank, \
    label_complete
...
def main():
    ...
    parser.add_argument('--split', default='train', help='The split of datasets')
    parser.add_argument('--test_on', default='five', help='The way of test,[five,one,prediction]')
    parser.add_argument('--margin', default=0.2, type=float, help='Rank loss margin.')
    parser.add_argument('--alpha', default=0.9, type=float, help='The weight of fuse instance and consensus')
    parser.add_argument('--adj_gen_mode', default='ReComplex', help='The mode of gen adj metrix')
    parser.add_argument('--t', default=0.3, type=float, help='The threshold of adj metrix')
    parser.add_argument('--re_weight', default=0.25, type=float, help='The weights of concepts in GCN')

    parser.add_argument('--Concept_label_ratio', default=0.40, type=float, help='The ratio of concept label.')
    parser.add_argument('--orig_data_name', default='coco', help='{coco,f30k}')
    ...
    parser.add_argument('--workers', default=40, type=int, help='Number of data loader workers.')
    ...
    parser.add_argument('--logger_name', default='./runs/coco/CVSE_COCO/log', help='Path to save Tensorboard log.')
    parser.add_argument('--model_name', default='./runs/coco/CVSE_COCO/', help='Path to save the model.')
    ...
    parser.add_argument('--attribute_path', default='data/coco_annotations/Concept_annotations_coco/',
    ...
    parser.add_argument('--inp_name',
                        default='data/coco_annotations/Concept_annotations_coco/coco_concepts_glove_word2vec.pkl',
                        help='load the input glove word embedding file')
    parser.add_argument('--adj_file', default='data/coco_annotations/Concept_annotations_coco/coco_adj_concepts.pkl',
    ...
    parser.add_argument('--lr_MLGCN_update', default=10, type=int, help='Number of epochs to update the learning rate.')
    parser.add_argument('--concept_name',
                        default='data/coco_annotations/Concept_annotations_coco/category_concepts.json',
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        if model.Eiters % opt.log_step == 0:
            logging.info('Epoch: [{0}][{1}/{2}]\t' '{e_log}\t'
...
def validate(opt, val_loader, model):
    ...
    img_embs, cap_embs, img_emb_cons, cap_emb_cons, concept_labels = encode_data(model=model, data_loader=val_loader,
                                                                                 alpha=opt.alpha)
    # remove KNN
    ...
    (r1, r5, r10, medr, meanr) = i2t_sep_sim(img_embs, cap_embs, img_emb_cons, cap_emb_cons, opt.data_name,
                                             weight_fused=opt.alpha, measure=opt.measure)
