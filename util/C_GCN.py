...
class C_GCN(nn.Module):
    ...
    def __init__(self, num_classes, in_channel=300, t=0, adj_file=None, norm_func='sigmoid', adj_gen_mode='C_GCN',
                 opt=None):
        ...
        _adj = gen_A_concept(num_classes, t, adj_file, opt.re_weight, gen_mode=adj_gen_mode)

        self.adj_O_P = Parameter(torch.from_numpy(_adj['adj_all']).float())
        self.adj_O_M = Parameter(torch.from_numpy(_adj['adj_all']).float())
