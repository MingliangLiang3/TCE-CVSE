...
def gen_A_concept(num_classes, t, adj_file, re_weight, gen_mode='ML_GCN'):
    import pickle
    result = pickle.load(open(adj_file, 'rb'))
    ...
    for key, value in result.items():
        if key == 'adj_O_P':
            ...
            if gen_mode == 'ML_GCN':
                ...
                _adj_OPC = _adj_OPC * re_weight / (_adj_OPC.sum(0, keepdims=True) + 1e-6)
            # only eq.(2) in paper
            ...
            if gen_mode == 'ML_GCN':
                ...
                _adj_OPC = _adj_OMC * re_weight / (_adj_OMC.sum(0, keepdims=True) + 1e-6)  #
            # only eq.(2) in paper
            elif gen_mode == 'My_rescale':
                '''Use My rescale function'''
            ...
            # combine eq.(2) and (3) in paper
                ...
                _adj_OMC[_adj_OMC < t] = 0
                _adj_OMC[_adj_OMC >= t] = 1
                _adj_OMC = _adj_OMC * re_weight / (_adj_OMC.sum(0, keepdims=True) + 1e-6)
            ...
            if gen_mode == 'ML_GCN':
                ...
                _adj_all = _adj_all * re_weight / (_adj_all.sum(0, keepdims=True) + 1e-6)
            # only eq.(2) in paper
            elif gen_mode == 'My_rescale':
                '''Use My rescale function'''
            ...
            _adj_all = _adj_all + np.identity(num_classes, np.int) * (
                        1 - re_weight)  # identity square matrix, re-weight
